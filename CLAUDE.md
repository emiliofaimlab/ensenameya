...
| `npm run check:*` | **Diecinueve comprobaciones ejecutables** sin red ni credenciales: `email`, `correos`, `terms`, `ics`, `chat`, `cadena`, `paypal`, `wise`, `stripe-payout`, `metodo`, `riel`, `rutas`, `enlaces`, `dinero`, `horario`, `cargo`, `csv`, `bots`, `seo`. Manda `package.json`, no este renglón: ya se quedó corto dos veces |
...
| **Google Analytics 4** (`components/analitica/google-analytics.tsx`) | Mide. `gtag.js` por `next/script`, sin dependencia nueva. ⚠️ **La variable va SOLO en el entorno Production de Vercel**: en "All Environments" los previews y `npm run dev` entran en los mismos informes y GA4 no sabe separarlos después. ⚠️ Las navegaciones internas del App Router las cuenta **GA, no el código**, por "Enhanced measurement › page changes based on browser history events": si alguien lo apaga en la consola, los embudos se quedan en una página y **no hay nada en el repo que lo delate** | `NEXT_PUBLIC_GA_ID` |
| **PostHog** (`instrumentation-client.ts`) | Mide producto. Arranca junto a Sentry, mismo fichero. `capture_pageview: "history_change"` —sin eso solo cuenta la primera carga— y `person_profiles: "identified_only"`, así que las personas las crea `PostHogIdentidad` al entrar a `(app)`, con el **id de Supabase y los roles, nunca el correo**. ⚠️ Esta clave **sí** puede ir en todos los entornos: cada evento lleva la propiedad `entorno`, que es por donde se filtra. 🔴 **El SESSION REPLAY viene ENCENDIDO por defecto y se queda encendido** (decisión de Jose, 22-sep): graba la pantalla del usuario y se enciende desde el PANEL de PostHog, no desde nuestro código, así que **nada en el repo lo delata** —se descubrió mirando las peticiones del navegador, `us.i.posthog.com/s/`—. Los `input` se enmascaran solos; **el chat NO**, porque es texto del DOM. Por eso la política de cookies lo declara: si alguien toca ese interruptor en el panel, el texto legal y el código dejan de coincidir **y ningún check puede verlo** | `NEXT_PUBLIC_POSTHOG_KEY` (+ `NEXT_PUBLIC_POSTHOG_HOST` si el proyecto es de la región EU — la clave **no funciona cruzada**) |
| **Google Calendar** (`lib/google-calendar.ts`) | Escribe el evento **al instante** en el calendario de quien conecta su cuenta en `/account` (el feed suscribible sigue para Apple/Outlook y tarda 8-24 h). Lo dispara un trigger en `sessions`/`bookings` por `pg_net` → `/api/calendario/google/sync` (`20260925180000`). El id del evento **es** el de la sesión sin guiones: no hay tabla de correspondencias. El refresh token va **cifrado** con una clave derivada de `GOOGLE_CLIENT_SECRET`: rotarlo obliga a reconectar. ⚠️ `calendar.events.owned` es un permiso **sensible**: sin la verificación de Google, «app no verificada» y tope de 100 usuarios. ⚠️ `pg_net` no reintenta: un fallo queda en los logs de Vercel y se corrige en el siguiente cambio de la sesión | `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` (+ la URI `https://<origen>/api/calendario/google/callback` registrada en Google Cloud) |
| **Google Search Console** (`app/robots.ts` · `app/sitemap.ts`) | Indexa, **desde el 22-sep**. Verificación por etiqueta HTML y no por TXT en el DNS: ese dominio lleva M365 tras Proofpoint y el correo no se toca. ⚠️ `robots.txt` se abre **solo en producción** (`VERCEL_ENV`); los previews siguen cerrados o compiten con el dominio bueno en el índice. Qué queda fuera lo dice `lib/seo.ts`, y lo vigila `npm run check:seo` | `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` |
