...
| **PostHog** (`instrumentation-client.ts`) | Mide producto. Arranca junto a Sentry, mismo fichero. `capture_pageview: "history_change"` —sin eso solo cuenta la primera carga— y `person_profiles: "identified_only"`, así que las personas las crea `PostHogIdentidad` al entrar a `(app)`, con el **id de Supabase y los roles, nunca el correo**. ⚠️ Esta clave **sí** puede ir en todos los entornos: cada evento lleva la propiedad `entorno`, que es por donde se filtra | `NEXT_PUBLIC_POSTHOG_KEY` (+ `NEXT_PUBLIC_POSTHOG_HOST` si el proyecto es de la región EU — la clave **no funciona cruzada**) |
