...
| **Wise** | ✅ **DESBLOQUEADO el 4-sep-2026**: token de API vivo contra `api.transferwise.com` — perfil business `136151426`, saldos y **presupuestos** responden ($150 → 456.354,38 COP, comisión $4,06). El camino de pago está abierto; **no hizo falta sandbox**. ⚠️ Sin probar: crear la transferencia, que Wise protege con SCA (firmar con una clave registrada). Ya no espera cuenta — **espera adaptador**, como Connect antes del 4-sep | `WISE_API_TOKEN` |
