...
| **B2** | Evitar saltos de pantalla | 🟢 **HECHO** (31-ago) | ⚠️ **Esta celda decía lo contrario y bloqueó el arreglo tres veces.** El ancla `#reservar` no evitaba el salto: lo **causaba** — al llevarla el href, cada selección disparaba el `scrollIntoView()` del App Router y realineaba el panel bajo la cabecera *sticky*. El arreglo es `scroll: false` en los tres controles que viven **dentro** del panel (`booking-select.tsx:136`, el `<Link>` del día en `booking-panel.tsx:374` y `add-to-cart.tsx:170`), donde el panel está en pantalla por definición; el aviso de «bajo el pliegue» solo aplicaría a enlaces que vengan de **fuera** —los dos de `carrito/page.tsx:659,675`—, que se dejan intactos con su ancla | N/A |
