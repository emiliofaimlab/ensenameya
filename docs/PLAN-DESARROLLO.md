...
- **R29-03** — `/pagos` es card-on-file del alumno (RN-43) y su copy lo dice; no tiene sentido en el
  ...
  forma la define el PSP (C-01 ✅ DLocal+Stripe; ⚠️ lo de «**sin cuentas/API keys**» era de julio —
  las dos cuentas están operativas desde el 4-sep-2026). Así que:
...
✅ **Ya no.** Verificado el 4-sep-2026 contra producción: `/terms` responde **200**.
...
## Panel del tutor v2 — paquete de Emilio (8-sep) · ✅ **APLICADO Y AUDITADO (9-sep-2026)**

Las **siete pantallas** del panel del tutor rehechas contra la lista cerrada que aprobó Emilio Faim
el 8-sep. En `dev` y en `main` (`4779c36`), **sin una sola migración** — la propia lista lo exigía y
se cumple: 165 migraciones antes y después.

- **Qué manda:** `docs/27-PANEL-TUTOR-V2-CAMBIOS-APROBADOS.md` (la lista, copiada al repo).
- **Cómo se aplicó y qué se decidió por el camino:** `docs/27-PANEL-TUTOR-V2-EJECUCION.md`.
- **Las 7 decisiones que esperan a Emilio** (DP-1…DP-6 y AB-06): §27b.8 de ese mismo doc. Ninguna
  bloquea; todas están implementadas «de la manera que hoy es cierta», con su `TODO` en el sitio.

**Auditado después de mergear**, no antes: 15 agentes de solo lectura —8 auditores, refutadores
independientes y un crítico de completitud— midieron **131 puntos** contra la lista y las capturas.
El refutador tumbó **2 de 9** incumplimientos (uno de ellos porque la condición estaba copiada de la
implementación de referencia del propio Emilio). Los reales se corrigieron el mismo día; el detalle
está en §27b.9.

⚠️ **Lo que NO se entregó como pedía la §9 de la lista:** un **PR por pantalla** con su **captura
junto a la de `capturas/`**. Fueron tres commits directos sobre `dev` —el orden sí se respetó— y la
comparación visual **sigue pendiente y la tiene que hacer una persona**: toda la verificación de
este trabajo son medidas del DOM, no capturas.
