...
async function csvTutores(from?: string): Promise<string> {
  ...
  return aCsv(
    ...
    filas.map((t) => [
      ...
      t.categorias.join(" · "),
      t.academia,
      t.suspendido ? "suspendido" : t.baja ? `baja ${t.baja.estado}` : t.estadoAprobacion,
      fechaCsv(t.aprobadoEl),
      t.identidad,
      t.documentos.map((d) => `${d.estado}: ${d.n}`).join(" · "),
      t.notasAprobacion,
      t.mentoriasPublicadas,
      t.mentoriasDetalle.map((m) => `${m.estado}: ${m.n}`).join(" · "),
      t.precioDesde === null ? "" : importeCsv(t.precioDesde),
      t.precioHasta === null ? "" : importeCsv(t.precioHasta),
      t.moneda,
      t.franjasDisponibles,
      t.impartidas,
      t.noShows,
      t.canceladas,
      t.alumnosDistintos,
      t.alumnos.map((a) => `${a.nombre} (${a.mentorias})`).join(" · "),
      fechaCsv(t.primeraClase),
      fechaCsv(t.ultimaClase),
      fechaCsv(t.proximaClase),
      t.notaMedia,
      t.resenas,
      t.tier,
      t.tierSplitPct,
      dineroCsv(t.generado.map((g) => ({ ...g, importe: g.bruto }))),
      dineroCsv(t.generado.map((g) => ({ ...g, importe: g.neto_tutor }))),
      dineroCsv(t.generado.map((g) => ({ ...g, importe: g.comision }))),
      t.metodoDeCobro,
      t.cuentaConfigurada ? "sí" : "no",
      t.cuentaPais,
      // ⚠️ Los cuatro últimos y nada más. El número de cuenta y el documento
      // de identidad NO salen de la base (`20260917180000`), y este fichero
      // —que se descarga, se reenvía y acaba en la carpeta de descargas de
      // alguien— es exactamente el motivo por el que no salen.
      t.cuentaUltimos4,
