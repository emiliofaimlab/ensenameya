...
/**
 * Excepciones puntuales del tutor (194:218).
 *
 * §3.4 (paquete v2, 8-sep-2026) · LA LISTA PRIMERO y el formulario detrás, que
 * abre «+ Añadir» desde la cabecera de la tarjeta. Antes el alta iba en un
 * `<details>` al pie: el orden da igual mientras haya una excepción, pero con
 * cinco el tutor tenía que recorrer el formulario entero para llegar a lo que
 * venía a mirar. Lo que se consulta va delante; lo que se rellena de vez en
 * cuando, detrás y plegado.
 */
export function ExceptionsManager({
  ...
  /**
   * §3.4 · título de la tarjeta. Lo pinta este componente por lo mismo que en
   * `AvailabilityManager`: el botón «+ Añadir» comparte fila con el título y
   * abre un formulario cuyo estado vive aquí.
   */
  titulo?: string;
}) {
  ...
  async function addException(e: FormEvent<HTMLFormElement>) {
    ...
    setEnd("");
    setReason("");
    // Se cierra al guardar: la excepción recién creada aparece arriba, en la
    // lista, y dejar el formulario abierto la tapaba.
    setAbierto(false);
    router.refresh();
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* `aria-controls` con el `id` del formulario: `aria-expanded` a secas
            anuncia «expandido» sin decir qué, y aquí además la lista se mete
            entre el botón y el formulario. */}
        <Button
          type="button"
          variant="outline"
          size="lg"
      ...
      {abierto ? (
        <form
          ...
          className="grid gap-3 rounded-[12px] border border-[#e0e0e0] p-4 sm:grid-cols-2"
        >
